...
const parseMML = function(data, fileName, cfg){
  let Layers;
  if (cfg == null) { cfg = {}; }
  if (cfg.layers == null) { cfg.layers = process.env.MAPNIK_LAYERS; }
  if (cfg.layers != null) { // A layers file
...
const layerParsers = {
  xml(d){ return d; },
  mml(d,fn, cfg){ return parseMML(JSON.parse(d), fn, cfg); },
  yaml: parseYMML,
  ymml: parseYMML
};
...
const loadCfg = function(layer, cfg){
  ...
  if (layer.id == null) { layer.id = path.basename(fn, ext); }
  ...
  if (layer.name == null) { layer.name = layer.id; }
