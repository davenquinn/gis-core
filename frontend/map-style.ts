...
class MapStyle {
  static initClass() {
    ...
    this.stylesheetDir = path.join(process.env.REPO_DIR,'map-styles','styles');
  ...
  constructor(opts){
    ...
    if (this.srs == null) { this.srs = opts.srs; }
    if (opts.layers == null) { opts.layers = []; }
    if (opts.styles == null) { opts.styles = []; }
    ...
    this.Layer = opts.layers.map(id=> {
      ...
      if (lyr.srs == null) { lyr.srs = this.srs; }
      console.log(lyr);
      return lyr;
    });
  ...
  __getNamedStyle(id){
    ...
    if (id.endsWith('.mss')) {
      ...
      if (existsSync(id)) { sourceFile = id; }
    }

    if (sourceFile == null) { sourceFile = path.join(this.constructor.stylesheetDir, `${id}.mss`); }
