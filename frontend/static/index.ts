...
class StaticMap {
  constructor(size, bbox, style, extraCfg){
    ...
    */
    ...
    this.size = size;
    this.style = style;
    if (extraCfg == null) { extraCfg = {}; }
    this.imageScale = extraCfg.scale || 2;
    this.scaleMap = extraCfg.scaleMap || false;
    const MPATH = process.env.MAPNIK_STYLES;
    if (this.style == null) { this.style = "ortho"; }
    if (extraCfg.layers == null) { extraCfg.layers = process.env.MAPNIK_LAYERS; }

    //# Paths to extra CartoCSS stylesheets
    if (extraCfg.styles == null) { extraCfg.styles = []; }
    ...
    this._map = new mapnik.Map(this.size.width*this.imageScale, this.size.height*this.imageScale);
    ...
    if (bbox == null) { bbox = [200,200,-200,-200]; }
    this.setBounds(bbox);
    this.__render = opts=> {
      return new Promise((res, rej)=> {
        ...
        console.log(w,h);
        if (this.scaleMap) {
          if (opts.scale == null) { opts.scale = this.imageScale; }
        }
  ...
  createFeatures(classname, opts){
    ...
    if (opts == null) { opts = {}; }
    if (opts.geographic == null) { opts.geographic = true; }
    const pathGenerator = opts.geographic 
                ? this.geoPath : this.path;
    const container = this.dataArea;

    const fn = {};
    fn.data = function(data){
      // Returns selection
      const el_ = container.append('g')
        .attrs({class: classname});
      const sel = el_.selectAll(`path.${classname}`)
        .data(data);
  ...
  transform(d){
    return [(d[0]-this.extent[0])*this.scale,(this.extent[3]-d[1])*this.scale];
  }

  inverseTransform(d){
    return [(d[0]/this.scale)+this.extent[0],
  ...
  __setupGeoPath() {
    ...
    const trans = d3.geoTransform({point(x,y){
      const v = _proj([x,y]);
  ...
  geoPath(d){
    if (this._geoPath == null) { this._geoPath = this.__setupGeoPath(); }
  ...
  path(d){
    ...
    if (this._path == null) { this._path = this.__pathGenerator(); }
    return this._path(d);
  }
  ...
  render(fn, opts){
    ...
    if (!fileExists(fn) || overwrite) {
      const im = this._map.renderSync({format: 'png', scale: this.imageScale});
  ...
  renderToObjectUrl(variables){
    if (variables == null) { variables = {}; }
    return this.filename = this.__render({format: 'png', variables})
      .then(im => new Promise(function(res,rej){
      console.log("Encoded");
  ...
  create(el, opts){
    ...
    if (opts.variables == null) { opts.variables = {}; }
    ...
    const p = this.filename.then(filename=> {
      ...
      this.image = el.append('image')
        .attr('xlink:href', filename)
        .attrs(this.size);

      this.dataArea = el.append('g')
        .attrs({
          class: 'data-area',
          'clip-path': "url(#mapClip)"
      });

      this.overlay = el.append('g')
        .attrs({
          class: 'overlay',
          'clip-path': "url(#mapClip)"
      });
      ...
      if (opts.scale.width == null) { opts.scale.width = this.size.width/3; }
  ...
  async createTiled(el){
    ...
    return __range__(minX, maxX, true).map((x, i) => __range__(minY, maxY, true).map(function(y,j){
      ...
      return renderer.serve(null, {z,y,x}, function(err,buffer){
        ...
        const im = el.append('img')
          ...
          .styles({
            ...
            height: sa,
            top: 0,
            left: 0}).on('load', () => URL.revokeObjectURL(uri));
